...
1. Create a separate Railway service for Worker 2.
2. Set the root directory to `worker2`.
3. Set the env vars above in Railway.
4. Keep `DRY_RUN=true` for the first deploy.
...
6. Watch logs. You should see the spot SOL balance, SOL price, planned sell amount, and planned USDC spot-to-perp transfer.
7. After logs look correct, set:
